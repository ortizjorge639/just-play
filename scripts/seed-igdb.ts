...
async function main() {
  ...
  for (const game of allGames.values()) {
    ...
    sqlLines.push(
      `INSERT INTO public.games (id, name, genres, estimated_session_length, header_image, description, source)` +
        ` VALUES ('${id}', '${name}', ARRAY[${genresArray}], ${sessionLength}, '${imageUrl}', '${description}', 'igdb')` +
        ` ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, genres = EXCLUDED.genres,` +
        ` estimated_session_length = EXCLUDED.estimated_session_length, header_image = EXCLUDED.header_image,` +
