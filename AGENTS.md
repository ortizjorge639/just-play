...
> Note: `.github/copilot-instructions.md` was rewritten for v2 on 2026-07-06 (at Jorge's request) and now summarizes the current architecture. This file remains the source of truth for branch state and process; that one is just a codebase summary.
...
### 🌿 2026-07-10 (later): the plan of record is now UPGRADE, not re-ship — branch `upgrade/v1-to-v2-merge`

Jorge reconciled the v2 question: v2 was never meant to be a wholesale swap. **v1's navigation/user flow (3-tab SPA: Deck → Session → Progress, settings modal, welcome/onboarding) is canonical and stays.** v2's frontend/functional improvements get layered on top, selectively. The work lives on `upgrade/v1-to-v2-merge` (off `main`); do NOT merge it into `main` without Jorge's explicit review + yes, and do NOT blind-merge `v2` into anything.

Key facts discovered while building the upgrade (verify fresh, as always):

- **v2's screens are mock prototypes, not functionality.** Every `app/(v2)/*` screen except Completed is `'use client'` with hardcoded MOCK_* data and unwired buttons. v1 is the functionally complete app (real server actions, sessions, XP, IGDB search).
- **v2 never actually served its own home.** `app/page.tsx` (v1 home) exists identically on v2 and silently wins the route over `app/(v2)/page.tsx` — v2-as-deployed was v1's home + v2's replaced bottom-nav + mock screens. That nav replacement was the UX regression behind the rollback.
- The real adoptable v2 assets: the treehouse feature (`components/treehouse-world.tsx`, `lib/treehouse.ts`, the Completed screen client), the version-in-title scheme, config hygiene (`.gitignore` tsbuildinfo, `pnpm-workspace.yaml`), and the visual design language of the mocks (as reference material).
- Design review with Jorge (2026-07-11, screen by screen): Session screen approved as-is after the cartridge-buddy integration; Progress gained the "Your Crew" buddy row (real data); the add flow stays booster-pack-first (v1's search already exists inside the deck — `game-search.tsx` is fully wired); the backlog mock was adopted as a real **`/backlog` route** ("Library", light-lavender mock style, real `game_progress` data, entry: the Games stat tile on Progress).
- The treehouse was adopted as a **new route `/treehouse`** (server page + client, ported from `app/(v2)/completed/`), reachable from a card on the v1 Progress tab — bottom-nav untouched. It includes the `fix/treehouse-empty-state` guards (branch `9f6ef9c`, Hermes's fix: empty-games crash + swallowed-error logging) on top of the script-race fix.
...
- **Post-ship follow-ups:** (a) **Vercel is NOT auto-deploying pushes** — after the merge landed, just-play-five.vercel.app was still serving a build from ~June 12 (the static `/welcome` page's CDN `age` header showed ~24 days). Jorge confirmed creds are stable, so the gap is the deploy trigger itself: check the Vercel project's git integration / production-branch setting. The tab title now carries the version + build patch (see Versioning below) — a deploy landed iff the live title's patch number changed. (b) `.github/copilot-instructions.md` was rewritten for v2 same day at Jorge's request. (c) the standing rule at the top of this section still applies to every **future** merge into `main`: this approval covered this merge only.

### Versioning (added 2026-07-06, Jorge's ask)

The tab title is `Just Play v<major.minor>.<buildPatch>`: `major.minor` comes from `package.json` (`2.0` as of this writing), and the patch is the UTC build timestamp `yymmddHHMM` computed in `next.config.mjs` at build time. Every deploy therefore shows a new, monotonically increasing patch number — compare the live title's patch against the latest push time to confirm a deploy actually happened. Both `app/layout.tsx` and `app/(v2)/layout.tsx` derive their `metadata.title` from `NEXT_PUBLIC_APP_VERSION`; keep them wired if you touch either.
...
- `feature/treehouse` is fully merged (its tip `1be183a` is an ancestor of `v2` — verified with `git merge-base --is-ancestor`). Deletion was attempted but blocked by the operating session's environment, and Jorge then said **"don't delete yet" (2026-07-06)** — so the branch stays on origin deliberately for now. Do not build on it, and don't delete it without a fresh ask.
...
- **`.github/copilot-instructions.md` was rewritten for v2 on 2026-07-06** — it's current again, but this file stays the source of truth for anything process- or branch-related.
